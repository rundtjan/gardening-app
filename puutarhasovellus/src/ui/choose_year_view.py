...
class ChooseYearView:
    ...
    def _add_choose_button(self, year):
        ttk.Button(master=self._frame, text=str(year), width=50, command=lambda: self._choose_year(year)).grid(column=0)

    def _initialize_year_buttons(self):

        for year in [self._current_year-1, self._current_year-2, self._current_year-3, self._current_year-4]:
    ...
    def _initialize(self):
        ...
        add_button = ttk.Button(master=self._frame, text="Show current year", width=50, command=lambda: self._choose_year(self._current_year))
