...
from services.gardening_service import (GardeningService, UserNameInUseError,
                                        CredentialsTooShortError, LoginError, PlantNameTooShortError, NoPlantingAmountError, YieldDateOrYieldAmountError)
...
class TestGardeningService(unittest.TestCase):
    def setUp(self):
        self._gardening_service = GardeningService(
            UserRepoStub(), plantation_repo_mock)
    ...
    def test_get_plantations_by_year_calls_repo_correctly(self):
        ...
        plantation_repo_mock.get_by_user_and_year.assert_called_with(self._gardening_service._user, '2021')
    ...
    def test_cant_create_plant_with_too_short_name(self):
        self.assertRaises(PlantNameTooShortError, self._gardening_service.create_plantation, "Tw", datetime.datetime.now(), "22kg", "Some info")

    def test_cant_create_plant_without_amount_planted(self):
        self.assertRaises(NoPlantingAmountError, self._gardening_service.create_plantation, "Beetroot", datetime.datetime.now(), "", "Some info")
    ...
    def test_cant_update_plant_with_too_short_name(self):
        plantation = Plantation('Exists', 'Tw', datetime.datetime.now(), '33kg', 'some info')
        self.assertRaises(PlantNameTooShortError, self._gardening_service.update_plantation, plantation)

    def test_cant_update_plant_without_amount_planted(self):
        plantation = Plantation('Exists', 'Reed', datetime.datetime.now(), '', 'some info')
        self.assertRaises(NoPlantingAmountError, self._gardening_service.update_plantation, plantation)

    def test_cant_update_plant_with_yield_amount_if_no_yield_date(self):
        plantation = Plantation('Exists', 'Reed', datetime.datetime.now(), '33kg', 'some info', None, '330kg')
        self.assertRaises(YieldDateOrYieldAmountError, self._gardening_service.update_plantation, plantation)

    def test_cant_update_plant_with_yield_date_if_no_yield_amount_and_not_flagged_ok_to_leave_out_amount(self):
        plantation = Plantation('Exists', 'Reed', datetime.datetime.now(), '33kg', 'some info', datetime.datetime.now(), '')
        self.assertRaises(YieldDateOrYieldAmountError, self._gardening_service.update_plantation, plantation)
